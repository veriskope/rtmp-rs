...
pub enum Message {
  //Placeholder,
  Command { name: String, id: f64, data: Value, opt: Vec<Value> },  // rename this opt to params?
  Response { id: f64, data: Value, opt: Value },
  Error { id: f64, data: Value, opt: Value },
...
impl Message {
  pub fn get_status(&self) -> Option<Status> {
    ...
    if let Message::Response { id:_, data:_, opt } = self {
          ...
          "onStatus" => {
            ...
            if let Value::Object(h) = opt {
              ...
              if let Some(status) = result {
                Message::Status { code: status.code.to_string(), 
  ...
  where T: AsyncWrite + Unpin
  {
    ...
    match msg {
      Message::Command { name, id, data, opt } => {
        ...
        for val in opt {
          Value::write(&mut writer, val).await.expect("write AMF optional info");
        }
      },
      _ => {
